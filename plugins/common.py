...
def getAssetOriginal(pluginName:str,assetFiltered:str)->List[str]:
    ...
    with PostgresConnectionContextManager() as cur:
        cur.execute("SELECT asset_original FROM asset_map WHERE asset_filtered=%s AND plugin_name=%s",(assetFiltered,pluginName))
        rows = cur.fetchone()
        if rows:
            return rows[0]
        else:
...
def registerAsset(pluginName,asset:Asset)->bool:
    ...
    store = Store()
    with PostgresConnectionContextManager() as cur:
        cur.execute("SELECT asset_filtered FROM asset_map WHERE asset_filtered=%s AND plugin_name=%s AND %s=ANY(asset_original)",(asset.assetFiltered,pluginName,asset.assetOriginal))
        rows = cur.fetchone()
        if rows:
            #重复资产，不录入
            return False
        cur.execute("SELECT asset_filtered FROM asset_map WHERE asset_filtered=%s AND plugin_name=%s",(asset.assetFiltered,pluginName))
        rows = cur.fetchone()
        #如果存在rows则将asset.original添加到该记录的asset_original字段数组内。如果不存在则新添加一个记录
        if rows:
            cur.execute("UPDATE asset_map SET asset_original=array_append(asset_original,%s) WHERE asset_filtered=%s AND plugin_name=%s",(asset.assetOriginal,asset.assetFiltered,pluginName))
        else:
            cur.execute("INSERT INTO asset_map (asset_filtered,asset_original,plugin_name) VALUES (%s,%s,%s)",(asset.assetFiltered,[asset.assetOriginal],pluginName))
    #查询一次该条记录，用hash表记录asset_original和asset_filtered的对应关系存入redis
    with PostgresConnectionContextManager() as cur:
        cur.execute("SELECT asset_filtered,asset_original FROM asset_map WHERE  plugin_name=%s AND asset_filtered=%s",(pluginName,asset.assetFiltered))
        rows = cur.fetchone()
...
def addAssetOriginalToTable(pluginName,asset_origianl):
    ...
    pluginName = pluginName.lower()
    with PostgresConnectionContextManager() as cur:
        cur.execute("SELECT asset_original FROM "+pluginName+"_table WHERE asset_original=%s",(asset_origianl,))
        rows = cur.fetchone()
        if rows:
            return False
        else:
            cur.execute("INSERT INTO "+pluginName+"_table (asset_original) VALUES (%s)",(asset_origianl,))
            return True
...
if __name__ == '__main__':
    registerAsset('test',Asset('http://www.bai12312311du.c1om','www.baidu.com'))
